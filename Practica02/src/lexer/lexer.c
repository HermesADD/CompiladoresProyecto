...
int lexer_scan(FILE *file) {
    ...
    while ((c = fgetc(file)) != EOF) {
        ...
        if (c == '/') {
            int next = fgetc(file);
            if (next == '/') {
                advance_position(next, &line, &column, file);
                
                int comment_c;
                while ((comment_c = fgetc(file)) != EOF) {
                    advance_position(comment_c, &line, &column, file);
                    if (comment_c == '\n' || comment_c == '\r') {
                        break;
                    }
                }
        ...
        if (is_identifier_start(c)) {
            char *lexeme = scan_identifier(file, c, &column);
            Token token;
            ...
            if (!token_init(&token, classify_identifier(lexeme), lexeme, token_line, token_column)) {
                free(lexeme);
                fprintf(stderr, "Error: no se pudo reservar memoria.\n");
                return 2;
            }
        ...
        if (scan_compound_operator(file, c, &compound_type, compound_lexeme, &column)) {
            Token token;
            if (!token_init(&token, compound_type, compound_lexeme, token_line, token_column)) {
                fprintf(stderr, "Error: no se pudo reservar memoria.\n");
                return 2;
            }
